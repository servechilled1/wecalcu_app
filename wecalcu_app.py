...
def page_calculatie():
    ...
    with st.expander("Totale Kosten"):
        ...
        col1.metric("Nettokostprijs", f"€ {cd.get('total_net_cost', 0.0):,.1f}")
        col2.metric("Opslag kosten", f"€ {cd.get('storage_cost', 0.0):,.1f}")
        col3.metric("Totale interne kosten", f"€ {cd.get('total_internal_cost', 0.0):,.1f}")
        col4.metric("Verkoopprijs (excl. BTW)", f"€ {cd.get('total_revenue_excl_vat', 0.0):,.1f}")
        col5.metric("BTW-bedrag", f"€ {cd.get('vat_amount', 0.0):,.1f}")
        col6.metric("Verkoopprijs (incl. BTW)", f"€ {cd.get('total', 0.0):,.1f}")
        col7.metric("Winst", f"€ {cd.get('total_profit', 0.0):,.1f}")
